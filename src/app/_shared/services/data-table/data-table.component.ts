...
export class DataTableComponent implements OnInit, OnDestroy {
  ...
  private handleSavedSearch() {
    // const isRoutedFromForm = this.appState.previousUrl.includes(this.tableUrl + '/watchers-form');
    // if (!isRoutedFromForm) {
  ...
  sort(column: DataTableColumn , dir: 'asc' | 'desc'): void {
    const existingIndex = this.criteria.sort.findIndex(option => option.column === column.name);

    if (existingIndex !== -1) {
      this.criteria.sort.splice(existingIndex, 1);
    }

    this.criteria.sort.push({
      column: column.name,
      direction: dir
    });
  ...
  isColumnSelected(column: string, direction: string): boolean {
    return this.criteria.sort.some(option => option.column === column && option.direction === direction);
  }
